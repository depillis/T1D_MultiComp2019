% run simulation for each value for fM and fMa parameters from
...
% Parameter list and values are from paramsmulti.m
...
% get the total number of values, second column should have the same amount
n = size(LHS, 1); 
...
setappdata(0, 'fMa_var', fMa);
setappdata(0, 'fM_var', fM);                    
...
[TnWave, YnWave, Te] = ode15s(@(t,y)rhs(t, y, fMas, fMs, wave(1)), Tspan, IC, options); % Solve ODE
...
[TnWave, YnWave, Te] = ode15s(@(t,y)rhs(t, y, fMas, fMs, wave(2)), Tspan, IC, options); % Solve ODE
...
%---------------------with wave = 1--------------------------%
options=odeset('Events',@sickEvents);
[TnWave, YnWave, Te] = ode15s(@(t,y)rhs(t, y, fMas, fMs, wave(3)), Tspan, IC, options); % Solve ODE
